...
All child repositories are public and tagged at `0.0.10`. Public child defaults
now resolve through public release artifacts:

- `swift-tui` is consumed through the `0.0.10` HTTPS SwiftPM tag.
- `swift-tui-web` publishes `@swifttui/web` and `@swifttui/build` tarballs on
  the GitHub `0.0.10` release.
- `swift-tui-examples` uses the `swift-tui` `0.0.10` tag and the web `0.0.10`
  release tarballs by default.
- `swift-tui-site` fetches tagged `swift-tui-examples` input into
  `.build/public-inputs/` and builds DocC from the `swift-tui` `0.0.10` tag.

As of `0.0.10`, `@swifttui/web` and `@swifttui/build` are published to npm and
...
The release version (currently `0.0.10`) is denormalized across every child:
...
```sh
mise run bump -- 0.0.10            # preview the full diff (dry run)
mise run bump -- 0.0.10 --write    # apply, then review `git -C <submodule> diff`
