...
Arguments:
  <new-version>   Target semver, e.g. 0.0.14 (also accepts pre-release suffixes,
                  e.g. 0.1.0-rc.1).
...
Examples:
  mise run bump -- 0.0.14                 # preview the full diff
  mise run bump -- 0.0.14 --write         # apply, then review `git diff` per repo
