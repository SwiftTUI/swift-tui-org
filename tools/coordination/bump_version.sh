...
Arguments:
  <new-version>   Target semver, e.g. 0.0.12 (also accepts pre-release suffixes,
                  e.g. 0.1.0-rc.1).
...
Examples:
  mise run bump -- 0.0.12                 # preview the full diff
  mise run bump -- 0.0.12 --write         # apply, then review `git diff` per repo
