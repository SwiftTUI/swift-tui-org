...
**13 of 15 opportunities landed + #4 shelved**, each built + tested + committed; the full `bun run test` repo gate is green at every boundary. Wave A + Wave B, Wave C **#12**/**#11**/**#9** (the `CompletionLedger` async-writable set, `029324e8`), and **#10's `ViewNode` field decomposition** (four value sub-structs, `8b59f095`) are all **merged to `swift-tui` `main`, pinned, and pushed**. **#4** is [formally shelved](2026-06-26-002-pipeline-ir-encapsulation-proposal.md#decision-2026-06-26). **Remaining: only #10's `ViewGraph` *method*-cluster extraction** (`GraphCheckpointStore`/…, a supervised pass).
