...
|---|---|
| **Date** | 2026-06-26 |
| **Status** | In progress — **11 / 15 landed + #9 started**. Wave A + Wave B (#1/#2/#3/#5/#6/#7/#8/#13/#14/#15), Wave C **#12** (PointerInteractionState), and the **#9** first step (`ConcurrentRegistrationCarry` + carry-forward totality) are all merged to `swift-tui` `main`, pinned, and pushed (each step build + full `bun run test` repo gate green). Remaining: #4 (IR encapsulation), Wave C #9-rest (`CompletionLedger`), #10, #11. See [Progress](#progress-2026-06-26). |
...
**11 of 15 opportunities landed (+ #9 started)**, each built + tested + committed; the full `bun run test` repo gate is green at the Wave A boundary, the Wave B boundary, after #15, after #12, and after each #9 step. Wave A + Wave B **and** Wave C #12 + the #9 first step are all **merged to `swift-tui` `main`, pinned, and pushed**.
...
| 4 | Retag the seven-phase pipeline IR from public to package before any god-object refactor | 🔴 high | ~~M~~ **XL** | ~~low~~ **med** | ⏸️ deferred — attempted; re-estimated XL (see Progress) |
...
| 9 | Decompose AnimationController and extract a CompletionLedger with a reusable carry-forward primitive | 🔴 high | L | medium | 🟡 first step done · `ConcurrentRegistrationCarry.swift` (swift-tui) |
| 10 | Decompose ViewGraph along its existing method-cluster seams and group ViewNode fields into aggregates | 🔴 high | XL | medium | ◻️ partial — `ViewGraph` already grouped (9 field groups + totality guard); `ViewNode` grouping remains (supervised) |
