...
**Hazard-guarding is therefore complete.** The only remaining true follow-on is the `@MainActor`-on-`FrameTailLayoutOffloadEligibility` part — that checker is reused by the worker (`renderLayoutAsync`), so isolating it is a worker-reuse cascade of the same class as #17, not a one-liner; tracked separately.
