...
**12 of 15 opportunities landed + #4 shelved + #9 first step + #10 ViewNode field-grouping**, each built + tested + committed; the full `bun run test` repo gate is green at every boundary. Wave A + Wave B, Wave C **#12** and **#11**, the **#9** first step, and **#10's `ViewNode` field decomposition** (four value sub-structs, `8b59f095`) are all **merged to `swift-tui` `main`, pinned, and pushed**. **#4** is [formally shelved](2026-06-26-002-pipeline-ir-encapsulation-proposal.md#decision-2026-06-26). Remaining: **#9-rest** (`CompletionLedger`, lower-urgency) and **#10's `ViewGraph` *method*-cluster extraction**.
...
| 10 | Decompose ViewGraph along its existing method-cluster seams and group ViewNode fields into aggregates | 🔴 high | XL | medium | ◻️ field-grouping done — `ViewGraph` (9 field groups) **and `ViewNode` (4 field groups, 25 fields, `8b59f095`)** grouped + totality-guarded; remaining: `ViewGraph` **method**-cluster extraction (`GraphCheckpointStore`/…) |
