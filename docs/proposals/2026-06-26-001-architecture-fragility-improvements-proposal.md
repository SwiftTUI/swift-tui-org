...
- **#4 was attempted and re-estimated M → XL.** A bulk `public → package` retag of the nine pipeline-internal `SwiftTUICore` subdirs was driven against the compiler across four build configurations (cascade of 705 / 532 / 246 / 384 errors). Findings: (a) the cascade is fully **contained to `SwiftTUICore`** — zero errors in Views/Runtime/Charts/Platforms, confirming no *external* consumer needs the IR; but (b) the seven phases are a **chained connected component** and the public consumer/host surface taps *multiple* phases — `Semantics` (the accessibility/focus API), `Draw` (`TextStyle`, shape payloads), `Animation` (the `Animatable*` family), `Raster` (surface), plus `@_spi` host exposures. So the survey's "~124 over-exposed types" is optimistic: most are *legitimately* public, and only the pure pipeline products (`ResolvedNode`/`MeasuredNode`/`PlacedNode`/`CommitPlan` + deep internals, ~30–40 types) are cleanly packageable — and even those are chained-exposed by the phase extractors, so converging to a correct boundary is a deliberate, multi-iteration design pass. A clean build *would* be correct-by-construction (the compiler forbids a consumer-exposed type from being `package`; the public-API-baseline diff is the review artifact), so the work is safe — just XL and low-urgency. **Reverted; no partial boundary shipped.**
