...
| 22 | Audit & enforce `Sendable` on the public modifier-type surface | High | M | Low | 🟡 in progress · layout+metadata groups `54c51f2b` |
...
**Group 1 done (2026-06-27, swift-tui `54c51f2b`).** Conformed the two densest value-config groups — **layout** (`Padding`, `SafeAreaPadding`, `IgnoreSafeArea`, `Border`, `Frame`, `Offset`, `Position`, `MatchedGeometry`, `FlexibleFrame`) and **metadata** (`ExactIdentity`, `LayoutMetadata`, `Horizontal`/`VerticalAlignmentGuide` — which already store `@Sendable` closures — `DrawMetadata`, `DrawEffect`, `SemanticMetadata`) = 16 types — to `Sendable`, compiler-verified. Added `PublicModifierSendableTests` as a **compile-time** guard (a metatype-only `requireSendable<T: Sendable>` assertion that fails the build if a listed modifier loses the conformance — the public-API inventory tracks members, not conformances, so it can't guard this; cf. #27's same finding). Full repo gate green. **Follow-on groups:** animation (`TransactionModifier`), focus (`PreferredDefaultFocus`/`DefaultFocusScope`/`BoolDefaultFocus`), gestures (`PointerHover`/`ContentShape`/`NamedCoordinateSpace`), and `IDModifier<ID: Hashable & Sendable>` (conditionally `Sendable`) remain; the generic-content and non-`@Sendable`-closure modifiers are the "reference escape" cases the proposal intentionally sets aside.
