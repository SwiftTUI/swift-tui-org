...
- **Not yet picked up (all need a dedicated session):** #24 (widen offload — sequenced after #16/#21, deliberately risky), #25 (release-pipeline automation — CI, npm-publish), #30 (`Layout.Cache` lifetime — public-API + behavior change).

**Bottom line:** every supplemental item is now either landed or carries a verified rationale; the autonomously-completable work is exhausted. The next remaining hardening/fragility option by the original leverage order is **#24 widen frame-tail layout-offload eligibility**; the rest of the dedicated-session queue is #25 release-pipeline automation, #30 `Layout.Cache` lifetime semantics, #31 faithful cross-host frame-encoder parity, plus optional low-value #29 enum consolidation / output-stability assertion.
