...
| 22 | Audit & enforce `Sendable` on the public modifier-type surface | High | M | Low | ✅ `Sendable` done (25 value modifiers, `54c51f2b`+`e7c61eb1`) · `+Equatable` follow-on |
...
**Remaining follow-on:** the proposal's optional `+ Equatable` on the pure-value subset (the closure-storing modifiers — `Transaction`, `PointerHover`, alignment guides — can't be `Equatable`, so it is a separate heterogeneous pass, not blocking the `Sendable` goal).
