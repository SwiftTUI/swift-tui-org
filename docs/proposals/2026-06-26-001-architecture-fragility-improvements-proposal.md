...
| 18 | Add an Android cross-compile gate to org CI alongside WASI | High | S | Low | ✅ done · `org-gate.yml` android-cross-compile job |
...
**Done (2026-06-27).** Added an `android-cross-compile` job to `org-gate.yml` mirroring the WASI job (validated `.swift-version`, swiftly toolchain, then `swift build --swift-sdk … --target SwiftTUIAndroidHost`, which transitively builds Core+Views+Runtime). **Verified locally first** — `swift build --swift-sdk swift-6.3.2-RELEASE_android --target SwiftTUIAndroidHost` compiles clean against the installed Android SDK, so no Android-only break exists at the current pin (my earlier "can't verify without an NDK" deferral was wrong — the SDK bundles its own NDK sysroot, and an Android Swift SDK was already installed). The one wrinkle the WASI job doesn't have: the Android SDK is published on its own cadence (no `6.3.1` Android SDK exists), so its version + URL + SHA256 (from swift.org's getting-started guide) are pinned **separately** from the toolchain — the 6.3.1 toolchain + 6.3.2 Android SDK pairing is the verified-working combination, with a comment to re-pin when a matching SDK ships for a newer toolchain.
