...
**Implementation status (2026-06-27, via the `/loop`).** The high-leverage, safely-completable items are landed and every remaining item is resolved or has a documented rationale, so the supplemental-implementation pass is effectively **concluded**:
- **Done:** #19 (bounded image caches), #28 (operator tests), #27 (error-type conformances), #22 (Sendable + Equatable on value modifiers), #21 (release-checked main-actor guard — hazard-guarding complete), #16 (mitigation-complete via #21), #20 (memo oracle always-on in DEBUG/test + sampled release), #23 (registry restore harness covers seam shapes + publication paths), #24 (safe frame-tail layout-offload widening for worker-safe layout-realized snapshots), #25 (release artifact pipeline hardening).
...
- **Not yet picked up (all need a dedicated session):** #30 (`Layout.Cache` lifetime — public-API + behavior change).

**Bottom line:** every supplemental item is now either landed or carries a verified rationale; the autonomously-completable work is exhausted. The next remaining hardening/fragility option by the original leverage order is **#30 `Layout.Cache` lifetime semantics**; the rest of the dedicated-session queue is #31 faithful cross-host frame-encoder parity, plus optional low-value #29 enum consolidation / output-stability assertion.
