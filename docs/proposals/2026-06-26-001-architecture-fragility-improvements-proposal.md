...
- **Done:** #19 (bounded image caches), #28 (operator tests), #27 (error-type conformances), #22 (Sendable + Equatable on value modifiers), #21 (release-checked main-actor guard — hazard-guarding complete), #16 (mitigation-complete via #21).
...
- **Investigated & declined/deferred with rationale:** #17 (FrameScheduler `@MainActor` = public-protocol cascade), #21-eligibility (pure scans → `@MainActor` adds no safety + cascades), #23 (faithful seam harness = multi-session; shallow nesting is false-confidence), #20 (`MemoSkipTrace` is an over-strict counter → no clean `unsoundSkip==0` oracle), #26 (`scopedAnyView` is `package`, so the "external authors" premise is off), #29 (test-half over-claimed — the 16-combo matrix already guards the fork; only cosmetic enum-consolidation remains), #31 (feasible but a focused-session two-wire-format effort, not autonomous).
...
**Bottom line:** every supplemental item is now either landed or carries a verified rationale; the autonomously-completable work is exhausted, and the remainder (#24/#25/#30 + #29-consolidation + #31-faithful-test + #20-oracle-refinement + #23-seam-harness) is dedicated-session work.
