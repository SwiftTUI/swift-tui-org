...
**13 of 15 opportunities landed + #4 shelved**, each built + tested + committed; the full `bun run test` repo gate is green at every boundary. Wave A + Wave B, Wave C **#12**/**#11**/**#9** (the `CompletionLedger` async-writable set, `029324e8`), and **#10's `ViewNode` field decomposition** (four value sub-structs, `8b59f095`) are all **merged to `swift-tui` `main`, pinned, and pushed**. **#4** is [formally shelved](2026-06-26-002-pipeline-ir-encapsulation-proposal.md#decision-2026-06-26). #10's `ViewGraph` method extraction is **underway** — `GraphCheckpointStore` (the checkpoint snapshot/compute reads) landed as a stateless operator (`75e5fd17`); the only piece left is its restore-*write* path + the `recordCheckpointGraphMutation` mutation-tracker cross-cut, a separately-gated pass.
...
| 10 | Decompose ViewGraph along its existing method-cluster seams and group ViewNode fields into aggregates | 🔴 high | XL | medium | ◻️ field-grouping done (`ViewGraph` 9 + `ViewNode` 4 groups, `8b59f095`); **`GraphCheckpointStore` extracted** — the checkpoint snapshot + mutation-state compute reads, a stateless caseless-enum operator (`75e5fd17`); remaining: the restore-*write* path + the `recordCheckpointGraphMutation` ~39-site mutation-tracker cross-cut (a separately-gated pass) |
