...
|---|---|
| **Date** | 2026-06-26 |
| **Status** | Original 15-item program effectively complete. Wave A/B/C work is merged, pinned, and pushed; **#4 (IR encapsulation) landed 2026-06-27** via `RenderSnapshot` + package-only phase artifacts after the source-breaking risk was accepted. See [Progress](#progress-updated-2026-06-27) and [proposal 002](2026-06-26-002-pipeline-ir-encapsulation-proposal.md#implementation-update-2026-06-27). |
...
**The original 15 opportunities are resolved/landed**, each built + tested + committed at its boundary. Wave A + Wave B, Wave C **#12**/**#11**/**#9** (the `CompletionLedger` async-writable set, `029324e8`), and **#10's `ViewNode` field decomposition** (four value sub-structs, `8b59f095`) are merged to `swift-tui` `main`, pinned, and pushed. **#4** is now [landed](2026-06-26-002-pipeline-ir-encapsulation-proposal.md#implementation-update-2026-06-27): the public renderer returns `RenderSnapshot`, raw phase products/engines are `package`, the public API baseline/policy encode the boundary, and the org root pins `swift-tui@315fd53c` plus `swift-tui-examples@3d70f26` (`swift-tui-org@73586ee`). **#10 is substantially done** — fields grouped, methods in ~13 extension files, and the cleanly-extractable pure-read operators (`GraphCheckpointStore` `75e5fd17`, `GraphNodeIndexQuery` `2c5bc1de`) lifted off the god class; the mutation-tracker deferral was investigated and declined (entangled, no payoff — see #10's row). Remaining ViewGraph polish is mutation-heavy logic correctly kept as extension methods.
...
- **Wave B — complete:** #6 `FeatureFlags` registry, #7 totality guards, #13 version-coherence gate, #14 bounded image caches, #15 autonomous-wake test **+ the `Tests/Support` harness extraction**, and #4 public IR encapsulation. #4 was re-estimated from M/mechanical to a source-breaking boundary pass, then landed after the risk was accepted.
...
Net: **#9, #10, #12 — and #11's fragility fix — could all land before #4.** (The original "#11 is genuinely gated" was found wrong on post-implementation verification: the `package`-DTO fix preserves the wire bytes, so only the *optional* public exposure of the DTO was #4-adjacent.) #4 has now landed for public-surface cleanliness and future refactor freedom, but it was a hard blocker for **none** of #9/#10/#11-fix/#12.
...
- **#4 was attempted twice and re-estimated M -> XL, then landed as a deliberate public-API boundary pass.** A bulk `public -> package` retag of the pipeline-internal `SwiftTUICore` subdirs was driven against the compiler across **~14 build configurations**, converging the in-`SwiftTUICore` cascade from 705 -> 16 errors by classifying the genuinely consumer/host-facing types and keeping them public (the `Animation` `Animatable*` family, `Draw`'s `TextStyle`/payloads exposed by the `@_spi` `DrawPayload`, `Raster`'s `RasterSurface`, `Pipeline`'s `FrameScheduler`/`Invalidating` that `RunLoop` requires, `EnvironmentSnapshot`, and the `@_spi` host surface). **The decisive finding:** once `SwiftTUICore` itself emits cleanly, the build reveals **~701 errors across 17 files of the *public Views authoring surface*** — `ViewPrimitives`, the lifecycle/metadata/layout **modifiers**, **gestures**, **focus** (`FocusedValue`), `ResolveContext`, `TabViewStyles`. So the survey's "no external consumer needs the IR" and "M / mechanical" were both wrong: the pipeline IR had leaked into consumer-facing API, and reducing it is source-breaking. The 2026-06-27 implementation accepted that risk and shipped the boundary: public renderer results are now `RenderSnapshot`, raw `FrameArtifacts`/phase nodes/phase engines are `package`, examples use public snapshot/raster/semantic surfaces, and the public API policy/baseline prevent regressions. See [`2026-06-26-002-pipeline-ir-encapsulation`](2026-06-26-002-pipeline-ir-encapsulation-proposal.md#implementation-update-2026-06-27) for commits and validation.
...
| 4 | Retag the seven-phase pipeline IR from public to package before any god-object refactor | 🔴 high | ~~M~~ **XL** | ~~low~~ **med** | ✅ done · `RenderSnapshot` facade + package-only phase IR (`swift-tui` `315fd53c`, examples `3d70f26`, root pin `73586ee`; see [proposal 002](2026-06-26-002-pipeline-ir-encapsulation-proposal.md#implementation-update-2026-06-27)) |
...
**Effort:** S < 1 day · M ~ days · L ~ 1–2 weeks · XL > 2 weeks. **Status:** original-program work is merged, pushed, and pinned through the org root unless an individual row says otherwise.
...
- **#4 Retag the seven-phase pipeline IR from public to package before any god-object refactor** — 🔴 high · XL · risk medium · done
...
- This began as a proposal; the status table and progress notes above now record the shipped work. Historical "first step" text is retained below for provenance.
...
**Implementation status (2026-06-28, via the `/loop`).** The high-leverage, safely-completable items are landed and every remaining item is resolved or has a documented rationale, so the supplemental-implementation pass is effectively **concluded**:
- **Done:** #19 (bounded image caches), #28 (operator tests), #27 (error-type conformances), #22 (Sendable + Equatable on value modifiers), #21 (release-checked main-actor guard — hazard-guarding complete), #16 (mitigation-complete via #21), #20 (memo oracle always-on in DEBUG/test + sampled release), #23 (registry restore harness covers seam shapes + publication paths), #24 (safe frame-tail layout-offload widening for worker-safe layout-realized snapshots), #25 (release artifact pipeline hardening), #30 (`Layout.Cache` pass-local lifetime contract).
- **Partial / blocked:** #18 (framework cross-compiles for Android — verified; CI gate blocked on a Linux NDK-sysroot path, reverted to keep `org-gate` green).
- **Investigated & declined/deferred with rationale:** #17 (FrameScheduler `@MainActor` = public-protocol cascade), #21-eligibility (pure scans → `@MainActor` adds no safety + cascades), #26 (`scopedAnyView` is `package`, so the "external authors" premise is off), #29 (test-half over-claimed — the 16-combo matrix already guards the fork; only cosmetic enum-consolidation remains), #31 (feasible but a focused-session two-wire-format effort, not autonomous).
- **Dedicated-session queue:** #31 faithful cross-host frame-encoder parity, plus optional low-value #29 enum consolidation / output-stability assertion.

**Bottom line:** every supplemental item is now either landed or carries a verified rationale; the autonomously-completable work is exhausted. The next remaining hardening/fragility option by the original leverage order is **#31 faithful cross-host frame-encoder parity**, plus optional low-value #29 enum consolidation / output-stability assertion.

### Ranked by leverage = (impact × fragility-removed) / (effort × risk)

| # | Opportunity | Impact | Effort | Risk | Status |
| --- | --- | --- | --- | --- | --- |
| 16 | Complete the LayoutProxyBox Mutex guard (finish #3 — structural, not just a trap) | High | S | Low | ✅ mitigation-complete via #21 (`a2eec874`); structural Mutex hook-banned |
...
| 18 | Add an Android cross-compile gate to org CI alongside WASI | High | ~~S~~ M | Low | 🟡 framework cross-compiles (verified); CI gate blocked on Linux NDK-sysroot |
| 19 | Bound the TerminalImageRenderer payload caches (kitty/sixel/fallback) | High | M | Low | ✅ done · `105414c1` |
| 20 | Promote the memo shadow oracle to always-on-in-test + sampled-release (finish #1) | High | M | Low | ✅ done · `55601200` |
| 21 | Guard/eliminate the `assumeIsolated` antipattern via one reusable checked-access helper | High | M | Low | ✅ hazard-guarding done (`247e4dc3`+`a2eec874`) · `@MainActor`-eligibility cascade deferred |
| 22 | Audit & enforce `Sendable` on the public modifier-type surface | High | M | Low | ✅ done · `Sendable` (25) + `Equatable` (20) value modifiers, `54c51f2b`+`e7c61eb1`+`1bc8b5b2` |
| 23 | Extend the generative registry-restore harness to seam shapes (finish #2) | High | L | Low | ✅ done · `55601200` |
| 24 | Widen frame-tail layout-offload eligibility (after 16/21 land the safety) | High | L | Med | ✅ done · safe layout-realized snapshot relayout, `3e09c2b2` |
| 25 | Harden the release artifact pipeline: automate npm publish + gate orchestration order/availability | High | M | Med | ✅ done · release artifact contract, `e456719` |
| 26 | Document & lint-enforce `scopedAnyView` adoption for deferred content | Med | S | Low | ⚠️ declined: package-only premise off |
...
| 29 | Consolidate the 5 perf-gate `Configuration` enums and test the untested observation-firing fork | Med | M | Med | 🟢 test-half over-claimed (matrix already covers it); only cosmetic enum-consolidation remains |
| 30 | Clarify & enforce custom `Layout.Cache` lifetime semantics | Med | M | Low | ✅ done · pass-local contract, `491f1314` |
| 31 | Add a cross-host frame-encoder parity test (WASI vs Android) | Med | M | Med | 🔬 feasible (both host-testable) but a focused-session effort (two wire formats; WASI format not Swift-decodable) |
...
**Resolution (2026-06-27): mitigation-complete; structural fix shelved.** #21's slice 1 (`a2eec874`) routed all five `LayoutProxyBox` accessors through the shared `withCheckedMainActorAccess`, whose `MainActor.preconditionIsolated` checks isolation **in release builds too** (a bare `assumeIsolated` does not) — so the off-main `cachedStates` race is now a loud, attributable crash in every build, which is the strongest mitigation achievable without the banned `Mutex`. The structural fix (the proposed `Mutex`) stays shelved: it requires the `cachedStates` values — opaque `Any` from a custom `Layout`'s associated `Cache` type — to be `Sendable`, which is a `Layout`-protocol public-API change (constrain `Layout.Cache: Sendable`), i.e. a large consumer-facing pass, not the "S" originally rated. **#16 is closed as mitigated;** the structural option is tracked only as a sub-note of any future `Layout.Cache` redesign.
...
### #18 — Android cross-compile gate in org CI — ✅ done (`org-gate.yml`)
...
**Partially done / re-rated S→M (2026-06-27).** Two findings from attempting it:

1. **The framework source cross-compiles clean for Android — verified locally.** `swift build --swift-sdk aarch64-unknown-linux-android28 --target SwiftTUIAndroidHost` (which transitively builds Core+Views+Runtime + the host's `#if os(Android)` code) compiles against the official Swift Android SDK (`swift-6.3.2-RELEASE_android`, URL+SHA256 from swift.org's getting-started guide). So **no Android-only break exists at the current pin**, and my earlier "can't verify without an NDK" deferral was wrong — the SDK bundles its own NDK sysroot and an Android Swift SDK was already installed.

2. **The Linux-CI gate is blocked on a host-specific NDK-sysroot include-path issue.** A job was added to `org-gate.yml` and run on the x86_64 Linux runner; it failed (twice) with `'semaphore.h' file not found` while building the `SwiftOverlayShims` C module — even though `semaphore.h` *is* present in the bundle (`ndk-sysroot/usr/include/`). It is the bundled NDK sysroot not being on clang's search path **from a Linux host** (it resolves fine from a macOS host). Two SDK wrinkles surfaced en route: (a) the Android SDK is on its own release cadence (no `6.3.1` SDK), so it must be version-pinned separately from the toolchain; (b) the bundle ID `swift-6.3.2-RELEASE_android` is ambiguous across 27 triples and SwiftPM auto-selects by *host* arch, so an explicit target triple is required. The job was **reverted to keep `org-gate` green** (a perpetually-red gate gates nothing). The robust fix is almost certainly `skiptools/swift-android-action` (the de-facto standard for Swift-on-Android CI, which handles the sysroot/NDK wiring — but a third-party action, a supply-chain call for the maintainer) or dedicated `-Xcc --sysroot`/`isystem` flags. That makes #18 an **M** (Linux-host sysroot setup), not the **S** originally rated. Re-pin command + triple recorded in the `org-gate.yml` header comment.
...
**Done (2026-06-27, swift-tui `55601200`).** `MemoSkipTrace` now defaults on in DEBUG/test builds and is release-available behind `SWIFTTUI_MEMO_TRACE`, with deterministic sampling controlled by `SWIFTTUI_MEMO_TRACE_SAMPLE` (default 64 in release, every frame in DEBUG). The observer is wired into `ViewGraph.beginFrame()` and the memo-computation shadow oracle is compiled in release, but trace-line emission remains opt-in so normal tests do not spam stderr. The reflective comparator and `ResolvedNode` memo oracle were moved out from `#if DEBUG` solely for this sampled diagnostic path; production memo reuse still uses the `Equatable`-only gate. `EquatableBoundaryReuseTests` now covers the sampled-frame latch and restores the trace globals around diagnostic assertions.

### #21 — Guard/eliminate the `assumeIsolated` antipattern with one reusable helper — 🟡 in progress (helper + 3 site-groups, `247e4dc3`)
...
**Why this matters more than "instrumentation" (2026-06-27 finding).** The hazard is concrete: `MainActor.assumeIsolated` **does not verify isolation in release builds**, so an un-guarded `nonisolated`→`@MainActor` bridge reached off-main (an eligibility mis-classification) proceeds and races *silently* in release — exactly the #1 SIGSEGV flake profile. LayoutProxyBox's `preconditionMainActor()` (`MainActor.preconditionIsolated`, which **does** check in release) is the only existing guard. So #21 is a release-safety fix, not cosmetic.

**Slice 1 done (2026-06-27, swift-tui `247e4dc3`).** Extracted `withCheckedMainActorAccess(_:_:)` (`package`, `SwiftTUICore/Support/`): `preconditionIsolated` (lazy diagnostic, so the only success-path cost over `assumeIsolated` is the executor check) then `assumeIsolated`. Routed LayoutProxyBox's five entry points through it (removing the local helper), and extended the release-time guard to the **two sibling offload disqualifiers the deep-dive named** — `LayoutDependentContentHandle` (2 bridges) and `IndexedChildSource` (4 bridges) — which had bare `assumeIsolated`. WASI cross-build + full repo gate green; success-path test added (the off-main trap can't be safely unit-tested).

**Slice 2 done (2026-06-27, swift-tui `a2eec874`) — per-site analysis of the remaining bridges.** The point of #21 is guarding the *worker-offload* hazard, not blanket-converting every `assumeIsolated`; converting the wrong site would add hot-path cost or trap valid off-main work. Analyzing the six remaining sites:
- **`HostedRasterSurface` `.assumedMainActor` — GUARDED.** A host that explicitly opts into "frames arrive on main" is the same assume-don't-verify hazard at a host boundary, and it is once-per-frame (cheap). Routed through the helper so a host contract breach is a loud crash. WASI-verified.
- **`Observation` `onChange` — intentionally bare (documented in code).** Fires on the observable's *mutation* context, which isn't guaranteed main, so a release precondition could trap a valid off-main mutation.
- **`Environment` ×3 (`recordEnvironmentRead`/`recordObservableRead`) — intentionally bare.** Evaluation-time dependency recording on a very hot path, not reachable by the frame-tail worker.
- **`RunLoop` `directWake` (Android) — intentionally bare.** A deliberately `@Sendable`-by-`@MainActor` wake-closure design (documented at the `AndroidDirectRunLoopPumpState` declaration); Android-only, untestable here.

**Hazard-guarding is therefore complete.** The remaining `@MainActor`-on-`FrameTailLayoutOffloadEligibility` sub-item was **investigated and declined (2026-06-27).** The eligibility methods (`canOffloadLayout`, `containsLayoutRealizedContent`, …) are **pure reads** of the `Sendable` `ResolvedNode` tree — they touch no `FrameTailRenderer` mutable state — and run on the *caller* side (the decision gate, before the worker dispatch in `renderLayoutAsync`). So there is no race for `@MainActor` to prevent; it would add **no** safety. And because `FrameTailRenderer` is `Sendable` (not `@MainActor`) and `renderLayoutAsync` is `async`, annotating these methods `@MainActor` would force `renderLayoutAsync` (and its render-pipeline callers) to `await`/main-isolate them — a cascade of the same class as #17, for zero safety gain. **Declined as net-negative**; the deep-dive's "future contributor cannot call it from a worker" rationale doesn't hold (pure functions over a `Sendable` input are worker-safe by construction).

### #22 — Enforce `Sendable` on the public modifier surface — 🟡 in progress (layout + metadata groups, `54c51f2b`)
...
**Scope correction (2026-06-27 audit).** There are exactly **60** `PrimitiveViewModifier` conformers, and they are *heterogeneous*, so the protocol cannot simply be refined to `: Sendable` and a single "all conformers are Sendable" totality guard is wrong: ~9 already declare `Sendable`; a large group is generic over `View` content (`OverlayModifier<OverlayContent: View>`, `BackgroundModifier`, the sheet/popover/menu/toast presentation modifiers, `BooleanNavigationDestinationModifier`, `GestureAttachmentModifier<G: Gesture>`) and cannot be unconditionally `Sendable` because the host `View`/`Gesture` content is not; and the lifecycle/preference modifiers store **non-`@Sendable`** closures. Real effort is M (per-group, compiler-gated), not a one-shot.

**`Sendable` goal done (2026-06-27, groups 1+2, swift-tui `54c51f2b` + `e7c61eb1`).** Conformed **25** value-type modifiers, compiler-verified, with `PublicModifierSendableTests` as a **compile-time** guard (a metatype-only `requireSendable<T: Sendable>` assertion that fails the build if any listed modifier loses the conformance — the public-API inventory tracks members, not conformances, so it can't guard this; cf. #27's same finding).
- **Group 1 (`54c51f2b`):** layout (`Padding`, `SafeAreaPadding`, `IgnoreSafeArea`, `Border`, `Frame`, `Offset`, `Position`, `MatchedGeometry`, `FlexibleFrame`) + metadata (`ExactIdentity`, `LayoutMetadata`, `Horizontal`/`VerticalAlignmentGuide` — `@Sendable` closures — `DrawMetadata`, `DrawEffect`, `SemanticMetadata`) = 16.
- **Group 2 (`e7c61eb1`):** animation (`TransactionModifier` — `@Sendable` closure), gestures (`PointerHover` — `@MainActor @Sendable` closure — `ContentShape`, `NamedCoordinateSpace`), focus (`PreferredDefaultFocus`, `DefaultFocusScope`), and the three generics whose parameters are already `Sendable`-bounded (`IDModifier<ID: Hashable & Sendable>`, `TagValueModifier<Value: Hashable & Sendable>`, `ValueAnimationModifier<Value: Equatable & Sendable>`) = 9.

**Intentionally excluded (reference escape):** generic-over-`View`-content modifiers (overlay/background/presentation/sheet/menu/popover/toast/navigation-destination/gesture-attachment — host content isn't `Sendable`); lifecycle/preference modifiers (store non-`@Sendable` closures); and the four focus *binding* modifiers (`BoolDefaultFocus`, `OptionalDefaultFocus`, `BoolFocusBinding`, `OptionalFocusBinding` — store a `FocusState.Binding` reference into mutable focus state).

**`+ Equatable` done (2026-06-27, swift-tui `1bc8b5b2`) — #22 fully complete.** Conformed the **20** pure-value modifiers (every `Sendable` modifier with no stored closure, no generic `View` content, no `FocusState.Binding`) to `Equatable`, compiler-synthesized — every payload type was already `Equatable` (`EdgeInsets`, `BorderSet`, `MatchedGeometryConfig`, `ProposedDimension`, `DrawMetadata`, `DrawEffect`, `SemanticMetadata`, `Path`, `Animation`, `Identity`, …), so no reverts. Excluded: `LayoutMetadataModifier` (its `LayoutMetadata` payload is `Sendable`-only) and the closure-storing modifiers (`Transaction`, `PointerHover`, the alignment guides). Extended `PublicModifierSendableTests` with a metatype-only `requireEquatable` compile-time guard over all 20. Full repo gate green.
...
**Done (2026-06-27, swift-tui `55601200`).** The generated restore oracle now enumerates seam cases rather than only sibling shapes: 9 seam kinds across 2...4 siblings and every changed sibling index, crossed with both diffed `.all` publication and a true `.subtrees` frontier publication. That yields 162 deterministic cases, including group/ForEach splice, portal/overlay/lazy-tab, sheet-captured, lazy-viewport, and identity-reroot islands. Each case still asserts scoped restore equals full rebuild across the broad registry families, so the harness now covers the seam and publication paths the supplemental item identified.
...
**Done (2026-06-27, swift-tui `3e09c2b2`).** The existing worker-safe pieces were already narrower than the proposal implied: indexed child sources were snapshotted before worker layout, and `SendableLayout` custom layouts already used worker proxies; ordinary non-`SendableLayout` custom layouts correctly remain main-actor-only. This pass closes the layout-realized content gap by keeping the first realization on the MainActor, then building a worker-safe snapshot for single-realized-child boundaries and relayouting that snapshot on the frame-tail worker when `FrameTailLayoutOffloadEligibility` accepts it. Diagnostics now accumulate layout-work and geometry-resolution metrics across the realization and worker relayout passes, so fallback evidence remains visible. Tests cover the new `GeometryReader` worker-relayout path and the unsafe custom-layout fallback that must stay inline.
...
**Done (2026-06-28):** `swift-tui-web/.github/workflows/publish.yml` now publishes from semver tags: it validates package versions, runs the Bun gate, packs both packages, checks that `@swifttui/build` depends on the exact same `@swifttui/web` version, uploads the tarballs to the GitHub release, and publishes the same tarballs to npm via trusted publishing/OIDC. The root `//:release_candidate` now includes `//:release_artifact_contract`, which reads `swift-tui-site/docs/releases.yml`, asserts the lockstep tag set exists, downloads and inspects the web GitHub release tarballs, checks npm package versions, verifies the Android GitHub Pages Maven AAR/plugin marker/plugin JAR and metadata, and runs a clean downstream Bun dry-run against the public tarball URLs. A fixture test covers the tarball metadata/dependency failure mode without live network.
...
**Investigated (2026-06-27): the test half is over-claimed.** `Tests/SwiftTUIViewsTests/ObservationFlagCombinationMatrixTests.swift` already enumerates **all 16 combinations** of the four flags (`PreciseObservationFiring`/`ObservableKeyPathInvalidation`/`ReaderAttribution`/`MemoReuse`) via a `withFlags` harness, resolves a fixed reader under each, and asserts the dependency edge is always recorded — i.e. the "essentially untested 4-way fork" is in fact guarded for its *substantive* risk (under-invalidation from an off-combination). So #29's stated "real fragility" is already covered; what remains is the **cosmetic enum-consolidation** (`FeatureGate` registry) the proposal itself rates lower ("not the enum sprawl"). Net: low remaining value — closeable, modulo an optional explicit raster-output-stability assertion on top of the existing matrix.
...
**Done (2026-06-28, swift-tui `491f1314`): resolved as an explicit pass-local contract.** The cache audit framed persistent custom layout cache reuse as a second design with actor-safety and memory-policy costs, so this pass deliberately did **not** add a `canReuseCache(from:to:)` hook that would imply durable cache state. Instead, the public `Layout.Cache` docs now state that SwiftTUI shares cache only from measurement to placement within one pass, exposes no cross-frame cache hook, and expects durable layout state to live outside `Cache`. The main-actor and `SendableLayout` erasure paths now name their discard helper `discardPassLocalCacheStates`, with comments at the map access points tying the implementation to the contract. Regression tests cover cache recreation after proposal, child-structure, and binding-driven invalidations on the main path, and proposal/structure changes on the frame-tail worker path, while preserving the same-pass measurement-to-placement reuse guarantee.
...
**Investigated (2026-06-27): feasible, but a focused-session effort, not an autonomous one.** Both encoders are **root-package, host-testable** targets (no `#if os(Android)` gate on `AndroidHostFrameEncoder`; `WASISurfaceBridge` runs host tests in the gate) routing through the shared `HostFrameProjection`, and `HostFrameProjectionContractTests.fullyPopulatedFrame()` is a reusable fixture — so the test is genuinely possible. The obstacles that make it a dedicated-session task: (1) the two wire formats differ structurally — Android emits JSON of a `Codable` snapshot (`encode → [UInt8]`), WASI emits a **custom delta-capable `String`** (`encode → String`) whose decoder lives in the TS web client, not Swift, so the WASI side isn't trivially round-trippable in a Swift test; (2) no existing test target imports both encoders, and adding both to one target must clear the WebHost/repository-split boundary checks (which list `WASISurfaceBridge` as a host target); (3) a shallow field-compare would be false-confidence (cf. #23). The faithful form compares the *projection both encoders derive* — i.e. assert each encoder's output preserves every `HostFrameProjection` field — rather than byte/format parity. Worth doing in a focused pass.
