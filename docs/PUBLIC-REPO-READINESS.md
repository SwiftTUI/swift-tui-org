...
- Each child repo has a pushed `0.0.1` tag.
- `swift-tui-web` has a public GitHub `0.0.1` pre-release with
  `swifttui-web-0.0.1.tgz` and `swifttui-build-0.0.1.tgz` assets.
- `swift-tui-examples` resolves `swift-tui` through the `0.0.1` HTTPS SwiftPM
  tag and resolves web packages through the `swift-tui-web` `0.0.1` release
  tarballs.
- `swift-tui-site` resolves DocC from the `swift-tui` `0.0.1` tag and fetches
...
The `0.0.1` cutover satisfies the public dependency shape using GitHub release
tarballs for the web packages. Migrating those two web dependencies from release
tarball URLs to npm package versions is a follow-up once npm credentials are
available.
