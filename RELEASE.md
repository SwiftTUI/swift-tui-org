...
> Worked example: this doc was written while cutting `0.0.14`. Substitute your
> target version for `0.0.14` throughout.
...
```sh
mise run bump -- 0.0.14            # preview (dry run)
mise run bump -- 0.0.14 --write    # apply
```
...
a. **Sync the lockfile's workspace versions.** `bun install` will NOT rewrite
   ...
   ```jsonc
   "packages/build": { "name": "@swifttui/build", "version": "0.0.14", ... }
   "packages/web":   { "name": "@swifttui/web",   "version": "0.0.14", ... }
   ```

   > **Why this matters:** `bun pm pack` / `bun publish` resolve `workspace:*`
   > against `bun.lock`, not `package.json`. If the lock lags, you publish
   > `@swifttui/build@0.0.14` depending on `@swifttui/web@<old>` — exactly the
...
c. **Build the tarballs** into a scratch dir (keep them out of the repo):
   ...
   Produces `swifttui-web-0.0.14.tgz` and `swifttui-build-0.0.14.tgz` — the exact
   filenames the release URLs reference. Verify the build tarball resolved its
   sibling correctly:

   ```sh
   tar xzO -f /tmp/swifttui-release/assets/swifttui-build-0.0.14.tgz package/package.json | grep swifttui/web
   # → "@swifttui/web": "0.0.14"
   ```
...
d. **Commit + tag + push** (release commits land directly on `main`; the org
   ...
   ```sh
   git add -A && git commit -m "0.0.14"
   git tag 0.0.14
   git push origin main && git push origin 0.0.14
   ```
...
e. **Create the GitHub release with both assets** — the step that was missing
   ...
   ```sh
   gh release create 0.0.14 --target main --title 0.0.14 --notes "..." \
     /tmp/swifttui-release/assets/swifttui-web-0.0.14.tgz \
     /tmp/swifttui-release/assets/swifttui-build-0.0.14.tgz
   ```

f. **Verify the tarball URLs resolve** (this is the 0.0.8 failure mode):

   ```sh
   for f in swifttui-web-0.0.14 swifttui-build-0.0.14; do
     curl -sS -o /dev/null -w "$f -> %{http_code}\n" -L \
       "https://github.com/SwiftTUI/swift-tui-web/releases/download/0.0.14/$f.tgz"
...
g. **Publish to npm** using the packed tarballs (so npm and the GitHub release
   ...
   npm publish /tmp/swifttui-release/assets/swifttui-web-0.0.14.tgz   --userconfig "$NPMRC" --access public
   npm publish /tmp/swifttui-release/assets/swifttui-build-0.0.14.tgz --userconfig "$NPMRC" --access public
...
a. **Bun** (web tarballs — required; the gate runs `bun install --frozen-lockfile`):
   ...
   bun install                      # fetches the 0.0.14 web tarballs, rewrites bun.lock
...
b. **SwiftPM** — regenerate every `Package.resolved` so the pinned revision
   ...
   `exact: "0.0.14"`; `swift package resolve` (resolution only, no full compile)
   updates each lock:
   ...
   bump the `swift-tui` `revision` (= `git -C ../swift-tui rev-list -n1 0.0.14`)
   and `version`.
...
`docs/releases.yml` (the bump sets it to `0.0.14`), then builds the WASI example.
...
# → "[prepare-webexample] prepared swift-tui-examples 0.0.14"  with no 404
```
...
- [ ] `swift-tui-web` has a `0.0.14` **git tag** and a **GitHub release** whose
      two `.tgz` assets return **HTTP 200**.
- [ ] `npm view @swifttui/web version` and `@swifttui/build` both show `0.0.14`.
- [ ] `swift-tui`, `swift-tui-examples`, `swift-tui-site` each have a `0.0.14` tag.
...
- [ ] `swift-tui-site` `bun run prepare:webexample` prints
      `prepared swift-tui-examples 0.0.14` with no 404.
- [ ] Root submodule pins recorded; `git submodule status` shows the new SHAs.

## Gotchas & notes

- **Tags are lightweight** here (`git tag 0.0.14`). SwiftPM and the GitHub release
